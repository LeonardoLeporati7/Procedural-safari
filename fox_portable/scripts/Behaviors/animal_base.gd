...
func _tick_fleeing(delta: float) -> void:
	...
	if _flee_targets.is_empty():
		_flee_timer -= 0.016  # fallback se non si passa delta
		if _flee_timer <= 0.0:
			_ensure_state(State.IDLE)
		return
	...
	if closest:
		...
		away.y = 0.0
		_move_in_direction(away.normalized(), sprint_speed, 0.016)
...
func _move_in_direction(dir: Vector3, speed: float, delta: float) -> void:
	...
	if body is CharacterBody3D:
		...
		cb.velocity.x = dir.x * speed
		cb.velocity.z = dir.z * speed
		cb.move_and_slide()
	else:
		body.translate(dir * speed * delta)
